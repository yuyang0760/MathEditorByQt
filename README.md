...
- **Paragraph（段落类）**：代表文档中的一个段落，由多个内容项组成
- **TextRun（文本片段类）**：代表具有相同格式的一段连续文本
- **MathObject（数学对象类）**：表示数学公式或其他数学元素
- **Format（格式类）**：定义文本的显示格式（字体、颜色、样式等）
- **Selection（选择类）**：表示文档中的选择区域，包含位置信息

模型层形成了一个典型的文档对象模型（DOM），其中Document包含多个Paragraph，每个Paragraph包含多个Item（TextRun或MathObject）。
...
- **TextRunItem（文本项）**：QGraphicsTextItem的子类，用于显示文本内容
- **MathObjectItem（数学对象项）**：QGraphicsItem的子类，用于显示数学公式
...
- **PositionUtils（位置工具类）**：提供位置计算和字符信息查询功能

### 📐 设计模式应用

1. **组合模式（Composite Pattern）**：Document-Paragraph-Item的层级结构
...
- **🔍 选择操作**：支持文本选择、全选等功能
- **🖱️ 鼠标交互**：支持鼠标点击定位和拖拽选择
- **⌨️ 键盘输入**：支持完整的键盘输入处理
- **📊 状态监控**：底部状态栏实时显示光标位置、选择状态等信息
- **📐 精确定位**：支持像素级的光标定位和坐标计算
